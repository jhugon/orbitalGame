...
class SpaceObjectModel:
  ...
  def update1(self,dt):
    ...
    for iEntry in reversed(list(range(len(self.burnSchedule)))):
      ...
      if self.burnSchedule[iEntry][1] <= 0.:
        self.burnSchedule.pop(iEntry)
      elif self.burnSchedule[iEntry][0] <= 0.:
...
class UniverseModel:
  def __init__(self,size,G=6.67e-11,rPower=-2.):
    """
...
class UniverseView(pygame.Surface):
  ...
  def showPaths(self,futurePaths,futureBurns,timePoints=None,selected=False):
    ...
    for objPath,objBurns,selectedPathBool in reversed(list(zip(futurePaths,futureBurns,selectedBools))):
...
class FuturePathsView(pygame.sprite.Sprite):
  def __init__(self, universeView):
    ...
    maxArrowAxis = max(self.pathSelectedStyle['arrowLength'],self.pathSelectedStyle['arrowWidth'])
    ...
    pygame.draw.polygon(self.arrowImg,self.pathStyle['arrowColor'],
          [
            [x-w2,y+l2],
            [x+w2,y+l2],
            [x    ,y-l2],
    ...
    pygame.draw.polygon(self.arrowImgSelected,
    ...
    if timeList != None and font != None:
      ...
      for pos,time in reversed(list(zip(pointList,timeList))):
        ...
        if time > 120.:
          time = time/60.
          append = "m"
        if time > 120. and append == "m":
          time = time/60.
          append = "h"
        if time > 48. and append == "h":
          time = time/24.
          append = "d"
        if time > 60. and append == "d":
          time = time/30.
          append = "Mo"
        if time > 23. and append == "Mo":
          time = time/12.
          append = "y"
...
class UniverseCtrl:
  ...
  def convertCoordsModel2View(self,x,y):
    return x/self.meterPerPixel+self.viewSize[0]/2,-y/self.meterPerPixel+self.viewSize[1]/2
  def convertCoordsView2Model(self,x,y):
    return (x-self.viewSize[0]/2)*self.meterPerPixel,(y-self.viewSize[1]/2)*self.meterPerPixel
  ...
  def run(self):
    ...
    while running:
      clock.tick(60)
      dt = clock.get_time()/1000. # Convert from ms to s
      counter += dt
      if counter > 2.:
        if self.debug:
            ...
            if self.selectedModeFlag and self.selectedBurnStartIndex != None:
              ...
              if endIndex != None and endIndex != self.selectedBurnStartIndex:
                ...
                if burnEndT > burnStartT:
                    self.selected[0].scheduleBurn(burnStartT,burnEndT,1.)
                if burnEndT < burnStartT:
                    self.selected[0].scheduleBurn(burnEndT,burnStartT,-1.)
              else:
                self.selectedBurnStartIndex = None
  ...
  def showPaths(self):
    ...
    if selectedModel==None or selectedModel.m>0.:
      selectedModel = None
    futurePaths, futureBurns = self.model.getFuture(timePoints,
                              selectedObj=selectedModel
                            )
    futurePathsView = []
    ...
    if selectedModel!=None and selectedModel.m == 0.:
  ...
  def isCloseToFuturePath(self,pos):
    ...
    x,y = pos
    dR2List = [(xS-x)**2 + (yS-y)**2 for xS,yS in self.selectedPathPointsView]
    minR2 = min(dR2List)
    result =  minR2 < drMax2
    if not result:
      return None
...
class SpaceObjectCtrl:
  def __init__(self, universe, img, scaleImg, x, y, mass=0.):
    """
...
class SpaceApplication:
  def __init__(self):
    pygame.init()
    windowsize = (800,600)
    backgroundImageLoc = "backgroundExt/night-sky-milky-way-galaxy-astrophotography_0p25.jpg"
    ...
    earth = SpaceObjectCtrl(universe,"sprites/planet3.png",0.5,0.,0.,mEarth)
    vehicle = SpaceObjectCtrl(universe,"sprites/FighterLaser_springgreen.png",1.,rVehicle,0.)
    vehicle2 = SpaceObjectCtrl(universe,"sprites/SatelliteBase16_red.png",1.,0.,rVehicle)
    vehicle3 = SpaceObjectCtrl(universe,"sprites/FrigateMissile_cyan.png",1.0,0.,-rVehicle)
